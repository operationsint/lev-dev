...
Public Class frmPRequest
    ...
    Sub lock_obj(ByVal isLock As Boolean)
        ...
        If m_PRequestId = 0 Then
            ...
            btnVoid.Enabled = isLock
            btnSubmitApproval.Enabled = isLock
            btnReject.Enabled = isLock
            btnApprove.Enabled = isLock
        Else
            txtPRequestNo.ReadOnly = True
            'If m_FrmCallerId = "frmPRequest" And p_UserLevel = 1 Then btnDelete.Enabled = Not isLock Else btnDelete.Enabled = False
            If m_PRequestStatus = "R" Then btnVoid.Enabled = Not isLock Else btnVoid.Enabled = False
    ...
    Sub clear_lvw()
        ...
        If m_PRequestId <> 0 Then
            ...
            Dim prm1 As SqlParameter = cmd.Parameters.Add("@prequest_dtl_id", SqlDbType.Int)
            prm1.Value = 0
            Dim prm2 As SqlParameter = cmd.Parameters.Add("@prequest_id", SqlDbType.Int)
            ...
            While myReader.Read
                lvItem = New ListViewItem(CStr(myReader.Item(1))) 'prequest_id
                lvItem.Tag = CStr(myReader.Item(0)) & "*~~~~~*" & intCurrRow 'ID
                'lvItem.Tag = "v" & CStr(DR.Item(0))
                For i = 2 To 3 'prequest_dtl_type, line_type
    ...
    Sub view_record()
        ...
        While myReader.Read
            ...
            isClosed = myReader.GetBoolean(10)
            If isClosed = True Then
                lblClosedDescription.Text = lblClosedDescription.Text & " " & myReader.GetString(11) & " on " & myReader.GetDateTime(12)
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            If m_PRequestId = 0 Then
                If txtPRequestNo.Text.Trim = "" Then
    ...
    Private Sub btnSaveD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveD.Click
        Try
            ...
            Dim prm17 As SqlParameter = cmd.Parameters.Add("@prequest_id", SqlDbType.Int)
            ...
            Dim prm21 As SqlParameter = cmd.Parameters.Add("@prequest_qty", SqlDbType.Decimal)
            ...
            If btnSubmitApproval.Enabled = False Then btnSubmitApproval.Enabled = True
    ...
    Private Sub btnApproval_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSubmitApproval.Click
    ...
    Private Sub btnCloseRequest_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCloseRequest.Click
        If MsgBox("Are you sure you want to close this Request?", vbYesNo + vbCritical, Me.Text) = vbYes Then
            cmd = New SqlCommand("usp_tr_prequest_CLOSE", cn)
            cmd.CommandType = CommandType.StoredProcedure
    ...
    Private Sub ntbPRequestQty_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles ntbPRequestQty.LostFocus
        If ntbPRequestQty.Text = "" Then ntbPRequestQty.Text = 1
        If ntbPRequestQty.DecimalValue = 0 Then ntbPRequestQty.Text = 1
        ntbPRequestQty.Text = FormatNumber(ntbPRequestQty.Text)
    End Sub

    Private Sub ntbPRequestQty_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ntbPRequestQty.TextChanged

    End Sub

    'Set autorefresh list---hendra
    Sub autoRefresh() 
        If Application.OpenForms().OfType(Of frmPRequestList).Any Then
            Call frmPRequestList.frmPRequestListShow(Nothing, EventArgs.Empty)
        End If
